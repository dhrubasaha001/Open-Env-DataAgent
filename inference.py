...
"""
    ...
    print(f"[START] task=data_analysis env=DataAnalysisEnv model={MODEL_NAME}", flush=True)
    try:
    ...
    print(f"[STEP] step=1 action={action} reward={reward} done={done} error=None", flush=True)
    print(f"[END] task=data_analysis success={reward > 0.0} steps=1 score={reward} rewards=[{reward}]", flush=True)
